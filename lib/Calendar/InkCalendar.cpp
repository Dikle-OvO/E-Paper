...
extern unsigned char imageBlack[15000]; // 视具体RAM大小调整，ESP32通常够用
extern unsigned char imageRed[15000];

extern Epd epd;
extern Paint paintBlack;
extern Paint paintRed;
...
void drawHeader() {
    ...
    // 计算今天是本月的第几天 (TODAY_DAY)
    // 计算月进度百分比 (0-100)
    int monthPct = (int)((float)TODAY_DAY / daysInMonth * 100);
    
    // 计算周进度 (0-6, 0=周日, 6=周六)
    // 先算出1号是周几
    int firstDayOfMon = getFirstDayOfWeek(CURRENT_YEAR, CURRENT_MONTH);
    // 算出今天是周几 (0-6)
    int currentWeekday = (firstDayOfMon + TODAY_DAY - 1) % 7; 
    // 转为 1-7 (视周日为第1天还是第7天，这里按经过的天数算，周日算1/7，周六算7/7)
    // 为了显示效果，我们假设一周进程是 (currentWeekday + 1) / 7
    int weekPct = (int)((float)(currentWeekday + 1) / 7.0 * 100);

    // 2. 布局配置
    int areaW = 130;              // 仪表盘总宽
    int startX = EPD_WIDTH - areaW - MARGIN_X;
    int topY = 20;                // 起始Y
    int barH = 6;                 // 进度条高度
    int barMaxW = 70;             // 进度条最大像素宽
    ...
    u8g2.setCursor(startX, topY + 6);
    u8g2.print("M.PRG");
    
    // 进度条槽 (空心黑框)
    paintBlack.DrawRectangle(startX + 35, topY, startX + 35 + barMaxW, topY + barH, 0);
    // 进度条填充 (实心黑块)
    int mBarFill = (int)((float)monthPct / 100.0 * barMaxW);
    if(mBarFill > 0) {
        paintBlack.DrawFilledRectangle(startX + 35, topY, startX + 35 + mBarFill, topY + barH, 0);
    }
    
    // 百分比数字
    u8g2.setCursor(startX + 35 + barMaxW + 5, topY + 6);
    u8g2.print(monthPct);
    u8g2.print("%");

    // --- 绘制周进度 (W.PRG) ---
    int row2Y = topY + 12; // 第二行Y坐标
    
    // 标签
    u8g2.setCursor(startX, row2Y + 6);
    u8g2.print("W.PRG");
    
    // 进度条槽 (空心黑框，保持风格统一)
    paintBlack.DrawRectangle(startX + 35, row2Y, startX + 35 + barMaxW, row2Y + barH, 0);
    
    // 进度条填充 (红色实心块，作为视觉高亮)
    // 注意：这里使用你测试出来的 1=红色
    int wBarFill = (int)((float)weekPct / 100.0 * barMaxW);
    if(wBarFill > 0) {
        paintRed.DrawFilledRectangle(startX + 35, row2Y, startX + 35 + wBarFill, row2Y + barH, 1);
    }
    
    // 如果周进度满了(周六)，加一个红色警告角标
    if (weekPct > 85) {
         paintRed.DrawFilledRectangle(startX + 35 + barMaxW + 2, row2Y, startX + 35 + barMaxW + 4, row2Y + barH, 1);
    }

    // 百分比数字
    u8g2.setCursor(startX + 35 + barMaxW + 5, row2Y + 6);
    u8g2.print(weekPct);
    u8g2.print("%");
